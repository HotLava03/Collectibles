...
public class SetDisplayNameCmd implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length == 0) {
            player.sendMessage(Colors.fixCodes("&9Usage &8&l\u00BB &7/setdisplayname <text...>"));
            return true;
        }
        ...
        meta.setDisplayName(Colors.fixCodes(String.join(" ",args)));

        item.setItemMeta(meta);

        player.getInventory().setItemInMainHand(item);
        player.sendMessage(Colors.fixCodes("&aSuccess &8&l\u00BB &7Display name set."));
        return true;
    }
}
